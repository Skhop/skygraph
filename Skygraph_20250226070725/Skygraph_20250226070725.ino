...
#define TmOpen  43   // open time 
#define TmClose  7  // close time 
#define LpNum  10 // multi seen times
#define TmReverse  400
...
int   processEnd;
int   processNum;
bool   proc0[150] ; // 1:west 0:east 
int    proc1[150] ; // time(sec)
bool   proc2[150] ; // shutter on
...
void loop(){
  ...
  for ( ii = 0; ii < processEnd ; ii++ ) {
    drawStatus(Tshot, loopNum , ii ,proc1[ii] );
    display.display();
    timer1 = timerSecond();
    Mdirection =  proc0[ii]  ;
